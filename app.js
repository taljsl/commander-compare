document.getElementById("add-deck").addEventListener("click", () => {
    ...
    newBox.innerHTML = '<textarea placeholder="Enter deck cards, one per line"></textarea>';
    container.appendChild(newBox);
  });
  
  document.getElementById("submit").addEventListener("click", () => {
    const decks = Array.from(document.querySelectorAll(".deck-box textarea"))
      .map(textarea => new Set(textarea.value.split("\n").map(card => card.trim()).filter(card => card)));
    ...
    result.deckStats.forEach(deck => {
      resultPage.document.write(`<div class="section"><h2>Deck ${deck.deckNumber} (${deck.totalCards} cards)</h2><pre>${deck.fullDecklist.join("\n")}</pre></div>`);
    });
